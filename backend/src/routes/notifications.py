...
def test_email_endpoint():
    ...
    """
    # This endpoint has complex validation logic
    # Consider implementing with @handle_errors() decorator after refactoring
    data = request.get_json() or {}
    email = data.get("email", "").strip()
    name = data.get("name", "").strip()

    if not email or not name:
        raise ValidationError("Les champs 'email' et 'name' sont requis")

    if "@" not in email or "." not in email:
        raise ValidationError(f"'{email}' n'est pas une adresse email valide")
    ...
    if success:
        ...
        raise ValidationError("L'email n'a pas pu être envoyé. Vérifiez les logs.")
