...
def download_ics(current_user, visite_id):
    ...
    if not visite:
        raise NotFoundError(f"Visite #{visite_id} inexistante")
    ...
    if not (is_vendeur or is_acheteur):
        raise ForbiddenError("Vous n'avez pas accès à cette visite.")
...
def modifier_visite(current_user, visite_id):
    ...
    if not data or (not data.get("date_heure") and not data.get("statut")):
        raise ValidationError("Veuillez fournir au moins 'date_heure' ou 'statut'.")
...
def obtenir_feedbacks_vendeur(current_user):
    ...
    if current_user.get("role") != "vendeur":
        raise ForbiddenError("Seuls les vendeurs peuvent voir le dashboard des feedbacks")
    ...
    if note_min is not None and (note_min < 1 or note_min > 5):
        raise ValidationError("note_min doit être entre 1 et 5")

    if note_max is not None and (note_max < 1 or note_max > 5):
        raise ValidationError("note_max doit être entre 1 et 5")
